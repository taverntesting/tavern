...
class YamlFile(pytest.File):
    ...
    def get_parametrized_items(self, test_spec, parametrize_marks, pytest_marks):
        ...
        for mark in parametrize_marks:
            if "key" in mark["parametrize"]:
                keys.append(mark["parametrize"]["key"])
            else:
                keys.extend(mark["parametrize"]["keys"])
        ...
        try:
            for mark in parametrize_marks:
                if "key" in mark["parametrize"]:
                    vals.append([
                        {mark["parametrize"]["key"]: v}
                        for v in mark["parametrize"]["vals"]])
                else:
                    vals.append(mark["parametrize"]["vals"])
            ...
            # mangled
            # TODO: fix
            vals = [[{k: "TAVERNERR" for k in keys}]]
            combined = itertools.product(*vals)

        combined = (functools.reduce(merge_dicts, x) for x in combined)

        inner_fmt = "-".join(["{{{}}}".format(k) for k in keys])
