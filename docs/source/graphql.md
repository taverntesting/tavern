...
- Must contain only `data` and/or `errors` at the top level
- Cannot have other top-level keys
- Both `data` and `errors` can be present, but at least one must exist
...
1. **Use GraphQL Variables**: Never use Tavern formatting in GraphQL queries - always use GraphQL variables
2. **Error Responses**: Test both successful responses and error scenarios
3. **Variable Types**: Ensure variable types match your GraphQL schema
4. **Operation Names**: Use descriptive operation names for complex queries
5. **Timeout Configuration**: Set appropriate timeouts for complex queries
6. **Authentication**: Test authenticated and unauthenticated scenarios separately
7. **Nested Testing**: Test nested data structures with realistic data volumes
...
graphql:
  client:
    ...
    ws_url: string                # WebSocket URL (future - subscriptions)
```